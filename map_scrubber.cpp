...
// Each element is a pair of fan and pump duty cycles (0-255)
// Logic: If exhaust is hot (i high), increase fan.
// If fluid is hot (j high), increase pump.
int candidates[16][16][2];

void init_candidates() {
    for (int i = 0; i < 16; i++) {
        for (int j = 0; j < 16; j++) {
            candidates[i][j][0] = i * 16 + 15; // Fan speed proportional to exhaust temp
            candidates[i][j][1] = j * 16 + 15; // Pump speed proportional to fluid temp
        }
    }
}
